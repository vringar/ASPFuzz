...
extern "C" fn on_vcpu(mut cpu: CPU) {
    ...
    write!(
        rs_file,
        "Until parse dir function:\t\t{:>12}/iter\n",
        format!("{:?}", total_time)
    )
    .unwrap();
    ...
    write!(
        rs_file,
        "Until copy pubkey function:\t\t{:>12}/iter\n",
        format!("{:?}", total_time)
    )
    .unwrap();
    ...
    write!(
        rs_file,
        "Until verify pubkey function:\t\t{:>12}/iter\n",
        format!("{:?}", total_time)
    )
    .unwrap();
    ...
    write!(
        rs_file,
        "Until load app function:\t\t{:>12}/iter\n",
        format!("{:?}", total_time)
    )
    .unwrap();
    ...
    write!(
        rs_file,
        "Until verify app function:\t\t{:>12}/iter\n",
        format!("{:?}", total_time)
    )
    .unwrap();
    ...
    write!(
        rs_file,
        "Until call off_chip function:\t\t{:>12}/iter\n",
        format!("{:?}", total_time)
    )
    .unwrap();
...
fn parse_args() -> Vec<String> {
    ...
    {
        println!(
            "{} not a valid Zen generation.",
            &cli_args.zen_generation.as_ref().unwrap()
        );
        std::process::exit(2);
    }
    ...
    unsafe {
        ZEN_GENERATION = Some(cli_args.zen_generation.unwrap());
    }
...
pub fn fuzz() {
    ...
    // Start emulator
    let emu = Qemu::init(&qemu_args, &env);
