...
type MyState  = StdState<BytesInput, InMemoryCorpus<BytesInput>, RomuDuoJrRand, CachedOnDiskCorpus<BytesInput>>;

pub fn run_client<SP>(emu: Qemu, state: Option<MyState>, solutions_dir: PathBuf, log_dir: PathBuf, input_dir: PathBuf, mut mgr: LlmpRestartingEventManager<(),MyState,SP>, mut harness: impl FnMut(&BytesInput) ->ExitKind) -> Result<(), Error> 
where
SP: ShMemProvider{
