...
parser = argparse.ArgumentParser(
    description="Check fuzzer solutions for known buffer overflow"
)
parser.add_argument(
    "run_dir",
    type=str,
...
for solutions_file_path in glob.glob(os.path.join(full_img_solutions_dir_path, "*")):
    ...
    found = False
    if (
        int.from_bytes(solutions_bytes[COMBO_DIR_ADDR : COMBO_DIR_ADDR + 4], "little")
        == COMBO_DIR_MAGIC
    ):
        dir_addr = DIR_ADDR
    else:
    ...
    for i in range(0, 64):
        ...
        dir_entry_magic = int.from_bytes(
            solutions_bytes[dir_entry_addr : dir_entry_addr + 0x4], "little"
        )
        # print(f"Entry magic = {dir_entry_magic:#010x}")
        if dir_entry_magic == RECOVERY_BL_MAGIC or dir_entry_magic == NORMAL_BL_MAGIC:
            entry_addr = (
                int.from_bytes(
                    solutions_bytes[dir_entry_addr + 0x8 : dir_entry_addr + 0xC],
                    "little",
                )
                & 0x00FFFFFF
            )
            ...
            entry_len = int.from_bytes(
                solutions_bytes[entry_len_addr : entry_len_addr + 0x4], "little"
            )
            if entry_len >= 0x80000000:
                known_counter += 1
                if dir_entry_magic == NORMAL_BL_MAGIC:
                    print(
                        f"Found in {solutions_file_path} at on-chip bootloader: {entry_len_addr:#010x}\n"
                    )
                elif dir_entry_magic == RECOVERY_BL_MAGIC:
                    print(
                        f"Found in {solutions_file_path} at recovery bootloader: {entry_len_addr:#010x}\n"
                    )
                found = True
                break
    if found:
